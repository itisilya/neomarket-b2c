...
import { Users, TrendingUp, DollarSign, ShieldCheck, Star, Heart } from "lucide-react";
...
export const ChannelCard: React.FC<ChannelCardProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-10 -right-10 h-28 w-28 rounded-full bg-cyan-500/5 blur-2xl group-hover:bg-cyan-500/15 transition-colors duration-300 pointer-events-none" />

      {/* Header Info */}
      <div className="relative z-10">
        <div className="flex items-start justify-between gap-2">
          <span className="inline-flex items-center rounded-lg bg-cyan-950/80 border border-cyan-900/40 px-2 py-1 text-[11px] font-bold text-cyan-400 uppercase tracking-widest text-shadow-sm/45">
          ...
          <div className="flex items-center gap-1.5 relative z-20">
            ...
            <button
              ...
              className={`flex h-7 w-7 items-center justify-center rounded-lg border transition-all duration-200 cursor-pointer ${
                isFavorite 
                  ? "bg-rose-950/80 text-rose-500 border-rose-900/50 hover:bg-rose-900/45 hover:text-rose-400" 
                  : "bg-slate-900/80 text-slate-400 hover:bg-slate-800 hover:text-rose-400 border-slate-800"
              }`}
